...
const crearCita = async (uid, data) => {
  ...
  if (!tallerId || !vehiculoId || !fecha || !hora) throw new Error("Faltan datos requeridos");
  ...
  if (usadas >= cupos) throw new Error("Cupo lleno");

  await db.collection("citas").add({
    tallerId, usuarioId: uid, vehiculoId, servicio,
    comentario, fecha, hora, estado: "pendiente", creadaEn: new Date(),
  });
};

const getCitasByUser = async (uid) => {
  const snapshot = await db.collection("citas").where("usuarioId", "==", uid).get();
...
const eliminarCita = async (id, uid) => {
  ...
  const docSnap = await docRef.get();
  if (!docSnap.exists) throw new Error("Cita no encontrada");
  if (docSnap.data().usuarioId !== uid) throw new Error("No autorizado");
  await docRef.delete();
};

const getCitasTaller = async (tallerId) => {
  const snapshot = await db.collection("citas").where("tallerId", "==", tallerId).get();
  const hoy = new Date();
  ...
  for (const docSnap of snapshot.docs) {
    ...
    try {
      if (data.usuarioId) {
        ...
        if (userDoc.exists) usuarioNombre = userDoc.data()?.datosPersonales?.nombre || "Cliente";
      }
    } catch { usuarioNombre = "Cliente"; }

    citas.push({
      id: docSnap.id, usuarioNombre,
      vehiculoId: data.vehiculoId || "",
      fecha: data.fecha || "", hora: data.hora || "",
      servicio: data.servicio || "", estado: data.estado || "pendiente",
    });
  }
...
module.exports = {
  getHorariosDisponibles, crearCita, getCitasByUser,
  cancelarCita, eliminarCita, getCitasTaller, confirmarCita,
};