const service = require("../application/taller.service");
...
const getCercanos = async (req, res) => {
  try {
    ...
    const data = await service.getTalleresCercanos(parseFloat(lat), parseFloat(lng));
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const getTallerById = async (req, res) => {
  try {
    ...
    if (error.message === "Taller no encontrado") return res.status(404).json({ error: error.message });
    res.status(500).json({ error: error.message });
  }
};
...
module.exports = {
  ...
  getCercanos, getTallerById, guardarConfiguracionCitas,
};