...
const updateTaller = async (uid, data) => {
  ...
  await db.collection("talleres").doc(uid).set(
    {
      datosPersonales: {
        nombre, telefono, direccion, imageUrl, domicilio,
...
const saveServicios = async (uid, data) => {
  ...
  if (!servicios || servicios.length === 0) throw new Error("Debes seleccionar al menos un servicio");
  await db.collection("talleres").doc(uid).set(
    {
      tipo: "taller",
      datosPersonales: { domicilio, actualizadoEn: new Date() },
      servicios,
    },
    { merge: true }
  );
...
const guardarConfiguracionCitas = async (tallerId, data) => {
  ...
  if (!intervalo || !cupos) throw new Error("Faltan datos");
...
module.exports = {
  createTaller, getTallerInfo, updateTaller, saveServicios,
  getTalleresCercanos, getTallerById, guardarConfiguracionCitas,
};