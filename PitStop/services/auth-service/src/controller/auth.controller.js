const service = require("../application/auth.service");
...
const login = async (req, res) => {
  try {
    ...
    if (error.message.includes("bloqueada")) return res.status(403).json({ message: error.message });
    if (error.message.includes("incorrecta")) return res.status(401).json({ message: error.message });
